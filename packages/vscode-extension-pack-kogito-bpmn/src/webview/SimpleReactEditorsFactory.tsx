...
import { EditorContainer } from 'sce-sim-grid';
// @ts-ignore
// const dmnJson = require('../../data/Traffic Violation2.dmn');
import * as dmnJson from '../../data/Traffic Violation2.dmn';
...
class ReactReadonlyEditor extends React.Component<Props, State> {
  ...
  public render() {
    ...
    console.log(content);
    console.log(`dmn:`);
    console.log(dmnJson.default);
    return (
      <React.Fragment>
      {(content && dmnJson) ? (
        <EditorContainer data={content} model={dmnJson.default} readOnly={false} />
      ) : <div>Loading</div>}
      </React.Fragment>
    );
  }
